...
def characterizeDual(lpd):
    ...
    #write dual to file
    #lpd.write('03.lp')
    lpd.write(os.path.join('lp', lpName+'_dual.lp'))
...
def binFluxInd(lppd, fix_lower_bounds, fix_upper_bounds, GPRdict, primalVariables, USE_GENE, binPrimal, binOn):
    ...
    # add lower bounds to all reactions that have not been included in the primal
    #for r_ in fix_lower_bounds:
    #    if USE_GENE:
    #        if r_ in GPRdict:
    #            lppd.indicator_constraints.add([[r_[0]], [1]], sense='GE', rhs=r_[1],
    #                                           indvar='bin_'+str(r_[0]), complemented=0, name='lb_'+str(r_[0]))
    #        else:
    #            print 'Lower bounds of {} cannot be changed in bilevel lp, because the reaction cannot be knocked out' .format(r_)

    #    else:
    #        lppd.indicator_constraints.add([[r_[0]], [1]], sense='GE', rhs=r_[1],
    #                                           indvar='bin_'+str(r_[0]), complemented=0, name='lb_'+str(r_[0]))

    # add upper bounds to all reactions that have not been included in the primal
    #for r_ in fix_upper_bounds:
    #    if USE_GENE:
    #        if r_ in GPRdict:
    #            lppd.indicator_constraints.add([[r_[0]], [1]], sense='L', rhs=r_[1],
    #                                           indvar='bin_'+str(r_[0]), complemented=0, name='ub_'+str(r_[0]))
    #        else:
    #            print 'Lower bounds of {} cannot be changed in bilevel lp, because the reaction cannot be knocked out' .format(r_)
...
def dualConsInd(lppd, dualConstraints, binPrimal):
    """
...
if __name__ == '__main__':
    ...
    dualConstraints, dualVariables = characterizeDual(lpd)
    ...
    lppd = dualConsInd(lppd, dualConstraints, binPrimal)
    #lppd.write('07.lp')
    ...
    # Write final MILP
    lppd.write(os.path.join('lp', lpName+'_bilevel_milp.lp'))
    MILP = cplex.Cplex(os.path.join('lp', lpName+'_bilevel_milp.lp'))
    ...
    try:
        if USE_GENE:
            delG = [x for x in sol if x.startswith('bin_S' or 'bin_s') and sol[x] == 0.0]
    ...
    if USE_GENE:
        ...
        F.write('\nDeleted reactions ({}): {}'.format(len(delReact), delReact))
    F.write('\n\nAnalysis time (minutes): {:f}'.format((time1-time0)/60.))
    F.write('\nMaximum deletions: {}'.format(maxDelete))
    F.write('\n\nUse knockout weighting (minimization): {}'.format(USE_KNOCKOUT_WEIGHTING))
    if USE_KNOCKOUT_WEIGHTING:
